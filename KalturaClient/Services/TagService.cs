...
namespace Kaltura
{
	...
	public KalturaTagService(KalturaClient client)
		...
		public KalturaTagListResponse Search(KalturaTagFilter tagFilter, KalturaFilterPager pager)
		{
			...
			_Client.QueueServiceCall("tagsearch_tag", "search", kparams);
			if (this._Client.IsMultiRequest)
				return null;
			XmlElement result = _Client.DoQueue();
			return (KalturaTagListResponse)KalturaObjectFactory.Create(result);
		}

		public int DeletePending()
		{
			KalturaParams kparams = new KalturaParams();
			_Client.QueueServiceCall("tagsearch_tag", "deletePending", kparams);
			if (this._Client.IsMultiRequest)
				return 0;
			XmlElement result = _Client.DoQueue();
		...
		public void IndexCategoryEntryTags(int categoryId, string pcToDecrement, string pcToIncrement)
		{
			...
			_Client.QueueServiceCall("tagsearch_tag", "indexCategoryEntryTags", kparams);
			if (this._Client.IsMultiRequest)
				return;
			XmlElement result = _Client.DoQueue();
		}
