...
namespace Kaltura
{
	...
	public KalturaCategoryService(KalturaClient client)
		...
		public int Index(int id, bool shouldUpdate)
		{
			...
			_Client.QueueServiceCall("category", "index", kparams);
			if (this._Client.IsMultiRequest)
				return 0;
			XmlElement result = _Client.DoQueue();
		...
		public KalturaCategoryListResponse Move(string categoryIds, int targetCategoryParentId)
		{
			...
			_Client.QueueServiceCall("category", "move", kparams);
			if (this._Client.IsMultiRequest)
				return null;
			XmlElement result = _Client.DoQueue();
			return (KalturaCategoryListResponse)KalturaObjectFactory.Create(result);
		...
		{
			KalturaParams kparams = new KalturaParams();
			_Client.QueueServiceCall("category", "unlockCategories", kparams);
			if (this._Client.IsMultiRequest)
				return;
			XmlElement result = _Client.DoQueue();
		}
		...
		public KalturaBulkUpload AddFromBulkUpload(FileStream fileData, KalturaBulkUploadJobData bulkUploadData, KalturaBulkUploadCategoryData bulkUploadCategoryData)
		{
			...
			_Client.QueueServiceCall("category", "addFromBulkUpload", kparams, kfiles);
			if (this._Client.IsMultiRequest)
				return null;
			XmlElement result = _Client.DoQueue();
