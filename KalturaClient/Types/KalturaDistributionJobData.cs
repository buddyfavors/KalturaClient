			...
			foreach (XmlElement propertyNode in node.ChildNodes)
			{
				...
				switch (propertyNode.Name)
				{
					...
					case "distributionProfile":
						this.DistributionProfile = (KalturaDistributionProfile)KalturaObjectFactory.Create(propertyNode);
						continue;
					case "entryDistributionId":
						this.EntryDistributionId = ParseInt(txt);
						continue;
					case "entryDistribution":
						this.EntryDistribution = (KalturaEntryDistribution)KalturaObjectFactory.Create(propertyNode);
						continue;
					case "remoteId":
						this.RemoteId = txt;
						continue;
					case "providerType":
						...
						continue;
					case "providerData":
						this.ProviderData = (KalturaDistributionJobProviderData)KalturaObjectFactory.Create(propertyNode);
						continue;
					case "results":
						this.Results = txt;
						continue;
					...
					case "mediaFiles":
						...
						{
							this.MediaFiles.Add((KalturaDistributionRemoteMediaFile)KalturaObjectFactory.Create(arrayNode));
						}
						continue;
				}
			}
